...
def process_ticket_data(ticket_id):
    ...
    if ticket:
        ticket_obj = (
            TicketLog.objects.filter(ticket_id=ticket_id)
            .order_by("-created_at")
            .first()
        )
        # if ticket_obj:
        ...
        process_ticket_embedding.apply_async(ticket_id, countdown=300)
...
def process_ticket_ai(self, ticket_id):
    # return "Skipping now"
    try:
        from ai.support_hub.ticket_classifier.ticket_classification import (
        ...
        if ticket:
            classification = run_ticket_classification(
                f"{ticket.title} \n\n {ticket.body}"
            )
            ...
            if classification:
                new_ticket = TicketLog.objects.create(
                    ...
                    type=Type.objects.filter(
                        type_id=classification.get("ticket_class", {})
                        .get("type", {})
                        .get("type_id", 0)
                    ).first(),
                    service=Service.objects.filter(
                        service_id=classification.get("ticket_class", {})
                        .get("service", {})
                        .get("service_id", 0)
                    ).first(),
                    priority=(
                        TicketPriority.objects.filter(priority_id=priority).first()
                        if priority
                        else None
                    ),
                    ...
                    ticket_state=TicketState.objects.filter(
                        state_id=1
                    ).first(),  # Assuming 1 is the ID for 'New' state
                    assigned_agent=(
                        Agent.objects.filter(agent_id=agent).first() if agent else None
                    ),
...
def _get_agent_by_queue_and_role(queue_id, role):
    ...
    agents = Agent.objects.filter(
        location__in=locations, role__icontains=role, is_valid=True
    )

    if agents.count() < 1:
        print(
            f"No agent found for queue {queue_id} with role '{role}'. Defaulting to agent_id=1."
        )
        return 1
    available_agent_ids = set()
    today = datetime.now().strftime("%a").lower()
    available_agents = Agent.objects.raw(
        f""" SELECT aa.agent_id FROM agents_agent aa 
    ...
    if available_agent_ids:
        ...
        if avail_qs.count() == 0:
            print(
                f"No agents on shift today for role '{role}'. Falling back to any matching agent."
            )
            return 1
